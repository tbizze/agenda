...
function submit() {
  router.get(route("recibos.index"), form);
}
/**
 * Função para abrir formulário de cadastro.
 */
function actionAdd() {
  router.get(route("recibos.create"));
}
/**
 ...
 */
 function actionCopy(id) {
  router.get(route("recibos.replicate", id));
...
function actionEdit(id) {
  router.get(route("recibos.edit", id));
}
/**
...
function actionDelete(id) {
  router.delete(route("recibos.destroy", id)); // Requisita ao controller a exclusão.
  isShowModal.value = false; // Fecha a modal.
}
/* Função para abrir PDF em nova aba.
 * Como parâmetro recebe o ID do registro.
 */
function openLinkNewTab(id) {
  const var_link = "recibos/" + id + "/pdf";
  ...
  <AppLayout :title="$props.titulo">
    ...
    <template #header>
      ...
      <BizButtonCreate
        ...
        label="Cadastrar"
        v-if="
          $page.props.auth.user.permissions.includes('evento-locals.create')
        "
      />
    </template>
        ...
        <table class="table table-compact">
          ...
          <tbody>
            ...
            >
              ...
              <td class="border-b ">{{ item.data }}</td>
              <td class="border-b ">{{ item.valor }}</td>
              <td class="border-b ">{{ item.pagador }}</td>
              <td class="border-b ">{{ item.recebedor }}</td>
              <td class="border-b space-x-1 w-40 text-center">
                <a class="border rounded-md py-1 px-1 bg-gray-50 hover:bg-gray-200 hover:font-bold " href="javascript:void(0)" @click="openLinkNewTab(item.id)">PDF</a>
                <BizButtonCopy
