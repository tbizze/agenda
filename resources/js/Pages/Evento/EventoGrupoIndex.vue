...
function submit() {
  router.get(route("evento-grupos.index"), form);
}
/**
 * Função para abrir formulário de cadastro.
 */
function actionAdd() {
  router.get(route("evento-grupos.create"));
}
/**
...
function actionEdit(id) {
  router.get(route("evento-grupos.edit", id));
}
/**
...
function actionDelete(id) {
  router.delete(route("evento-grupos.destroy", id)); // Requisita ao controller a exclusão.
  ...
  <AppLayout :title="$props.titulo">
    ...
    <template #header>
      ...
      <BizButtonCreate
        ...
        label="Cadastrar"
        v-if="
          $page.props.auth.user.permissions.includes('evento-grupos.create')
        "
      />
    </template>
